...
- Заголовок «Выберите задачу — подключите одну библиотеку» уменьшен и ограничен по ширине, чтобы сохранять три осмысленные строки и не перекрывать остальные элементы страницы.
...
- Следующий большой блок больше не повторяет определение платформы: теперь он ведёт от задачи приложения к одной библиотеке для Xcode.
...
- Для каждой из четырёх задач явно показано: какой один product добавить в Xcode и какие зависимости Xcode скачает сам.
