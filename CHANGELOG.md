...
## 2026-09-16 — Безопасное завершение брошенного RU checkout

- В account-policy инструкцию добавлены `RUCheckoutTerminationClientProtocol`,
  подключение client и вызов координатора после явной отмены
  пользователя.
- Описано, что только server-authoritative `failed`, `cancelled` или
  `expired` удаляет точный pending attempt и освобождает purchase/restore.
- Закрытие страницы, локальный таймаут и `expiresAt` оставлены
  fail-closed: они не исключают позднее списание. Добавлены ожидаемые
  UI-состояния и QA-сценарии.
- Таблица совместимости не изменена: BroadMonetization 4.1.0 появится
  в проверенном наборе только после публикации тега и release.
