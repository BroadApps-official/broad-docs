...
- тёмные объясняющие схемы во всех статьях стали значительно ниже:
  уменьшены карточки, текст, стрелки, внутренние отступы и тень;
- добавлена расширенная статья об интеграции Adapty: активация SDK, загрузка
  ...
  отдельная декоративная схема и блок «Перед чтением» над этой статьёй убраны,
  чтобы страница сразу начиналась с содержания;
...
- страницы Special Offer приведены к единому стилю и фактам финальной статьи
  «Спешл оффер (от Adapty)»: единый цикл окно оффера 24 ч → cooldown 24 ч с
  моментом отсчёта, сброс цикла при выключении флага, покупке или restore;
...
- Special Offer (RU и общий) переописан под реализацию с cooldown: показ идёт
  циклом «окно оффера → cooldown» (24 ч / 24 ч), обратный отсчёт ведёт до конца
  окна, а выключение флага, подтверждённая покупка и перевод часов назад
  сбрасывают цикл; прежняя формулировка «таймеров и cooldown нет» убрана;
