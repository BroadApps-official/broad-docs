...
на четыре части. Сначала перечислите **всё, что должно уметь приложение**, затем
добавьте в Xcode библиотеки для этих возможностей. Их может быть одна или
несколько. Общие зависимости выбранных библиотек Xcode скачает автоматически.
...
## Выберите библиотеки по задачам
...
Если приложению нужны возможности из нескольких строк, добавьте несколько
верхних products. Например, готовый paywall и HEX-утилиты означают
`BroadUIFlows` + `BroadExtensions`. Не добавляйте повторно `BroadCore` и
`BroadMonetization`, которые уже приходят вместе с `BroadUIFlows`.

Все комбинации объясняет [полная таблица выбора](./module-selection.md).

![Какая часть платформы отвечает за запуск, оплату, готовые экраны и утилиты](../public/guides/readme/architecture-light.svg)

## Подключите выбранные библиотеки в Xcode

Повторите шаги ниже для каждого верхнего product, который выбрали. Products,
указанные в колонке «придёт автоматически», отдельно не добавляйте.
...
6. Дождитесь, пока Xcode загрузит библиотеку без ошибок. Затем повторите шаги
   для следующей выбранной библиотеки или соберите приложение.
...
> **Пример.** Приложение передаёт ключ Adapty и имя `settings`. Платформа запрашивает
