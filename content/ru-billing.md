...
Спешл оффер с Apple, СБП или картой разобран отдельно. Разработчик получает
подтверждённые campaign/product values, подключает coupon-каталог, разделяет
два таймера и проверяет результат после Safari:
