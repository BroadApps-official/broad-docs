...
Этот раздел документации относится именно к RU Billing. Общие покупки через Adapty и StoreKit описаны в [BroadMonetization](./broad-monetization.md); другие подключаемые биллинги могут получить собственные разделы внутри монетизации.

## Статьи раздела RU Billing

| Задача | Статья |
|---|---|
| Подключить карту и СБП, проверить доступность и подтверждение оплаты | Эта страница |
| Получить и разобрать серверный каталог тарифов | [Продукты с backend](./backend-product-catalog.md) |
| Выбрать RU-продукт для второго предложения | [Special Offer — RU-продукт](./ru-special-offer.md) |
| Подготовить Special Offer без правки кода | [Инструкция менеджеру](./ru-billing-account-manager.md) |
| Подключить эксперимент в модуле | [A/B в платформе](./ru-billing-ab-platform.md) |
| Связать показ варианта с RU-оплатой | [A/B-тесты для разработчика](./ru-billing-ab-tests-developer.md) |
...
Обычный путь использует explicit `ru_pay=true` из текущего payload стандартного
Adapty SDK. SDK не сообщает приложению, пришёл ли payload из сети, managed cache
или Dashboard fallback, поэтому все три варианта имеют provider authority и
могут открыть RU Billing при `ru_pay=true`.

Это не относится к persistent cache BroadMonetization: сохранённый платформой
paywall может отрисовать тарифы offline, но не авторизует RU methods. `false`,
отсутствующее, некорректное или конфликтующее значение остаётся запретом.

Резерв при полной недоступности Adapty получает отдельное разрешение только для
текущего показа: он **не подставляет `ru_pay=true`** и не восстанавливает gate из
platform cache.
