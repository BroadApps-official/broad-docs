...
Если вы **аккаунт-менеджер** и вам нужно настроить кабинет, продукт или A/B-тест,
не продолжайте по разработческой инструкции. Откройте отдельную пошаговую
страницу: [«RU Billing: спешл оффер и A/B-тест — аккаунт-менеджеру»](./ru-billing-account-manager.md).

Если вы **разработчик** и задача именно про распределение пользователей между
вариантами, откройте [«RU Billing: A/B-тесты для разработчика»](./ru-billing-ab-tests-developer.md).
...
[«RU Billing: спешл оффер»](./ru-special-offer.md). В ней добавляется один gate
`kupon = true` и отдельный RU-продукт из платёжного каталога `nextgenwebapps`.
На примере 232 Claude строка `widgetTitle == "kupon"` возвращает
`monthly_12.99_nottrial` за 990 ₽. Этот RU-продукт не берётся из Adapty или
App Store. При `true` оффер показывается всегда, без таймеров и дополнительных
условий.
...
- [Аккаунт-менеджеру: спешл оффер и A/B-тест](./ru-billing-account-manager.md)
