...
| Готовые первые экраны, paywall и маршрутизация | `BroadUIFlows` |

Ключи, тексты, изображения, адреса backend и продуктовые правила всегда
...
5. Какие backend-методы нужны каждому пользовательскому сценарию?
...
- backend-запрос для каждого сценария;
...
сценарий, например «запуск → первый экран» или «открыть paywall → увидеть
продукты», и доведите его до рабочего состояния.
...
1. собирает требования, дизайн, reference и backend-контракты;
2. заполняет `AppIntegrationPlan.md`;
3. создаёт новый `iOS → App` или использует существующий target;
...
- каждый backend-сценарий работает на настоящих данных;
- ошибки, пустые ответы и отсутствие сети показаны понятно;
- paywall показывает все продукты, полученные от Adapty;
