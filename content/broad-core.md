...
С Core **2.0.0** приложение может записывать собственные события через
`BroadLogEvent.host(BroadLogHostEvent(...))`. Коды, имена полей и текстовые
значения объявляются как `StaticString`; значения также принимают `Bool` и
`Int`. Событие попадает в общие логгеры и support log. В собственном exhaustive
`switch` по `BroadLogEvent` добавьте `.host`.
[Контракт и пример Core 2.0.0](https://github.com/BroadApps-official/broad-core-ios/blob/2.0.0/README.md) ·
[Обновление зависимостей](./compatibility.md#обновление-с-набора-3-0-0-на-4-0-0).

## Стабильный аккаунт и Keychain

Для переустановки, смены телефона и сохранения серверных лимитов есть отдельная
[статья о Keychain и восстановлении аккаунта](./keychain-account-recovery.md).
Она описывает `KeychainAccountIdentifierStore` из Core 2.1.0, обработку ошибок
и подключение к оплате. В шаблоне 4.1.0 store сохраняет customer ID для Adapty;
баланс и авторизацию по-прежнему предоставляет backend приложения.
