...
С 3.0.0 этот маршрут есть в платформе: `resolveTokenCheckoutMethods` и
`startSelectedToken`, со свежим исходным балансом и account-policy проверкой
после закрытия оплаты или возврата. [Подключение карты/СБП для токенов](./ru-billing.md).
В собственном экране маршрутизируйте эти методы отдельно от Apple;
готовый `BroadTokenPaywallView` продолжает использовать Apple manager.
...
С BroadMonetization **4.0.0** fulfillment явно разделяет временный сбой и
окончательный отказ. `.failed`, `.unavailable` и `.pending` сохраняют прежнюю
попытку и evidence; Retry повторяет начисление без новой Apple-покупки.
`AppError.isRetryable == false` сам по себе не означает окончательный отказ.
Возвращайте `.rejected(error)` только при подтверждённом окончательном отказе
backend: после успешной очистки pending store он разрешает новую попытку.
При уже начисленной тому же аккаунту транзакции верните `.alreadyCredited`.
[Порядок обновления и проверки](./compatibility.md#обновление-с-набора-3-0-0-на-4-0-0).
...
Общий порядок входа и восстановления описан в [восстановлении аккаунта и покупок](./account-recovery.md), а постоянные серверные записи — в [данных аккаунта](./backend-account-data.md). Ниже — вызовы, относящиеся к token manager.
