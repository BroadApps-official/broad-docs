...
ОНБОРДИНГ ── последняя страница ──→ ОБЫЧНЫЙ PAYWALL
                                             ├─ purchase / restore подтверждены → MAIN
                                             └─ закрыт без покупки
                                                       ↓ если разрешено
                                                SPECIAL OFFER
                                                       ├─ подтверждён Premium → MAIN
                                                       └─ закрыт → MAIN

MAIN → SETTINGS → Upgrade / Restore / Support / Legal
```
...
| `BroadAppFlowView` | связывает onboarding, paywall и main |
...
| Special Offer | открывается только после закрытия обычного paywall без подтверждённой покупки или restore |
| Main | открывается после завершения бесплатного пути или подтверждённого Premium |
...
1. Пройдите весь путь onboarding → paywall → main.
