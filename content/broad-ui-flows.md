...
| Special Offer | после закрытия обычного paywall разрешающий флаг `true` показывает экран в окне показа; когда окно истекает, действует cooldown, после которого цикл повторяется |
