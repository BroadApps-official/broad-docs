...
![BroadUIFlowsGallery: выбран первый продукт; второй доступен ниже, кнопки Continue и Restore показаны отдельно](../public/guides/modules/ui-paywall.png)

Это экран BroadUIFlows над демонстрационным каталогом. Надписи `provider display value A/B` заменяют реальные цены: пример показывает порядок продуктов и выбранную карточку. Покупки в Gallery не выполняются; в приложении используйте настоящие данные провайдера.
