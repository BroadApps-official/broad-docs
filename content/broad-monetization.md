...
Модуль возвращает только числа (`ProductPricePresentation`); превращение их в надписи и бейджи остаётся за `BroadUIFlows` или собственным UI приложения. Продукт без корректной цены или с непонятным периодом просто не получает эти цифры. Массив продуктов, как и везде, не фильтруется и не сортируется.
...
Таймер 24 часа находится в UIFlows и сейчас является чистым визуальным циклом. Monetization не создаёт серверное расписание и не превращает ноль таймера в запрет покупки.
