...
1. Приложение просит placement, например `main`.
...
- кампания активна по умолчанию — её выключает только явный `special_offer = false`;
- экран должен быть собственным непустым экраном placement (fallback/подменённый main отклоняется);
- все продукты передаются интерфейсу без фильтрации;
- успешный purchase или restore первого paywall ведёт в main и обходит второй экран.

Таймер работает в двух режимах. **Без расписания** он остаётся визуальным циклом
24 часа. **С расписанием** (timed) модуль включает кадэнс «сутки через сутки»:
окно по доверенному серверному времени (`ServerSynchronizedSpecialOfferClock`),
показ на каждом закрытии в течение окна, затем cooldown; без доверенного времени
оффер не показывается. Модуль отдаёт состояние и данные; **когда** показать второй
экран, решает приложение. Подробнее — на странице [«Спешл оффер»](./special-offer.md).
