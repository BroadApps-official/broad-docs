...
подтверждённой покупки только по одному правилу: точное булево
`special_offer = true` в текущем Remote Config показывает второй экран, любое
другое значение не показывает его. При `true` экран показывается всегда.
Отдельный локальный таймер идёт по циклу
`24:00:00 → 00:00:00 → 24:00:00` и не управляет показом.
