...
> `special_offer = true`, а продукты второго
...
Обычный paywall
        ...
        └─ нажат крестик без покупки
                    ↓ если показ разрешён (точный boolean true)
              Special Offer
...
основного paywall. Циклический счётчик на экране остаётся только визуальным:
`24:00:00 → 00:00:00 → 24:00:00`. Ноль не закрывает оффер, не запрещает
покупку и не запускает cooldown.
...
false, нет поля или неверный тип      → не показывать второй экран
```
...
| Повторное открытие | Оффер появляется снова, а визуальный таймер продолжает локальный цикл без eligibility и cooldown |
...
| Таймер | визуальный цикл `24:00:00 → 00:00:00 → 24:00:00`; ноль не закрывает экран и не блокирует checkout |
