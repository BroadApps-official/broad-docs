...
## Цена за неделю и бейдж экономии — по желанию

По умолчанию карточка показывает название, подпись, цену и период ровно так, как
их вернул платёжный слой. Раньше, если дизайн просил «$1.15 в неделю» или «−40%»,
приложение считало эти цифры само.

Теперь это умеет сама платформа — опционально. В настройке paywall есть
переключатель `BroadPaywallPricePresentation`:

- `.disabled` (по умолчанию) — прежний вид, ничего не меняется;
- `.standard` — под ценой появляется недельная цена, а на выгодных тарифах —
  бейдж экономии и один бейдж best-value.

Цифры считаются из уже полученных продуктов (цена, приведённая к неделе, и
процент относительно самого дорогого недельного тарифа). Продукт без корректной
цены или с непонятным периодом просто не получает эти надписи вместо угаданного
числа. Тексты бейджей задаёт приложение, порядок и состав продуктов не меняются.
