...
Крестик только сообщает о закрытии обычного paywall. Для любого Special Offer
...
Для RU Billing продукт сначала заводят в российском кабинете, а его ID
сопоставляют с Adapty символ в символ. При `special_offer = true` второй экран
показывается всегда. Таймер относится только к UI: локальный цикл
`24:00:00 → 00:00:00 → 24:00:00` продолжается между открытиями и не влияет на
решение о показе или доступность покупки.

## Что проверить

| Проверяем | Ожидаемый результат |
|---|---|
| Крестик обычного paywall | Special Offer открывается только при `special_offer = true` в текущем Adapty Remote Config |
| Успешные purchase и restore | Второе предложение не открывается |
| Продукт RU Billing Special Offer | Заведён в RU Billing и совпадает с Adapty по точному product ID |
| Таймер Special Offer | Локально идёт `24 → 0 → 24`, между открытиями не сбрасывается и не закрывает экран на нуле |
