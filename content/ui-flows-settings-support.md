...
Напишите в поддержку: `<адрес поддержки текущего приложения>`». Адрес должен
быть виден целиком; рядом нужны действия «Скопировать email» и «Закрыть».
Не показывайте бесконечный loader и не отмечайте обращение как отправленное.
...
4. Support открывает чат или composer, а при недоступной почте показывает адрес
   поддержки и позволяет его скопировать; данные не отправляются автоматически.
