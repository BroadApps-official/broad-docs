...
const simpleVisuals: Record<string, SimpleVisualContent> = {
  ...
  "usedesk": {
    label: "ЧАТ ПОДДЕРЖКИ · ОТДЕЛЬНО ОТ ПЛАТФОРМЫ",
    title: "Чат загружается только после нажатия пользователя",
    steps: [
      ["01", "Открыть настройки", "пользователь нажимает «Онлайн-чат»"],
      ["02", "Получить его данные", "сервер связывает чат с текущим аккаунтом"],
      ["03", "Показать переписку", "история не смешивается между аккаунтами"],
    ],
    result: "Usedesk не замедляет запуск и не открывает чужую историю чата.",
  },
  "release-process": {
