...
const simpleVisuals: Record<string, SimpleVisualContent> = {
  ...
  "ru-billing-ab-tests-developer": {
    label: "RU BILLING A/B · ОДИН СЧЁТЧИК НА ПОКАЗ",
    title: "Вариант приходит из Adapty, показ и оплата встречаются на backend",
    steps: [
      ["01", "Adapty выбирает вариант", "распределение одно на все placement, код сегмента лежит в Remote Config"],
      ["02", "Продукты берутся из каталога", "id варианта ищутся в каталоге backend, иначе набор по умолчанию"],
      ["03", "Показ уходит в одну сторону", "RU-gate закрыт — событие в Adapty, открыт — assign и paywall-shown на backend"],
      ["04", "Конверсия считается по id", "платёж лежит под тем же пользователем, отдельное событие покупки не отправляется"],
    ],
    result: "Знаменатель и числитель оказываются в одном месте: показ ровно в один счётчик, никогда в оба.",
  },
  "onboarding-att": {
