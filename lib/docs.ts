...
export type DocEntry = {
  ...
  when: string;
  outcome: string;
  group: "Старт" | "Части платформы" | "Монетизация" | "RU Billing" | "Архитектура" | "Разработка";
  body: string;
};
...
export const docs: DocEntry[] = [
  ...
  { slug: "ru-billing", title: "Карта и СБП", description: "Когда показать российские способы оплаты, как проверить ru_pay и регион и почему возврат из браузера ещё не означает успех.", purpose: "Безопасно добавить СБП и карту рядом с Apple без ложного Premium.", when: "Когда подключаете, проверяете или обновляете RU Billing.", outcome: "Понятно, когда показать RU-способы, как провести checkout и чем подтверждается Premium.", group: "RU Billing", body: ruBilling },
  { slug: "backend-product-catalog", title: "Продукты с backend", description: "Где настраивается запрос, как разобрать JSON и сохранить все RU-тарифы без потерь.", purpose: "Подключить backend конкретного приложения к общей платёжной логике через готовый или собственный decoder.", when: "Когда backend отдаёт продукты для СБП и карты.", outcome: "Каждая строка каталога проверена, сохранена в исходном порядке и точно связана с продуктом Adapty/App Store.", group: "RU Billing", body: backendProductCatalog },
...
export const docGroups = ["Старт", "Части платформы", "Монетизация", "RU Billing", "Архитектура", "Разработка"] as const;
